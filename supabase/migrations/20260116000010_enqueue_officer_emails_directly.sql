-- Migration: Enqueue officer emails directly in approve_club_request function
-- This ensures officers ALWAYS get emails, bypassing any trigger issues
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.clubs c WHERE c.slug = v_club_slug) THEN
    RAISE EXCEPTION 'Slug already exists: %', v_club_slug;
  END IF;

  -- Create club (matching original schema)
  ...
  LOOP
    ...
    -- DIRECTLY ENQUEUE EMAIL FOR OFFICER (bypass trigger)
    -- Use same template as contact emails (we know it works)
    ...
    v_payload := jsonb_build_object(
      ...
      'dashboard_url', v_dashboard_url || '/' || COALESCE(v_club_slug, v_club_id::text),
      'role', 'officer'
    );

    -- Insert email directly into outbox
    ...
    VALUES (
      v_officer_email,
      'club_approved_contact',  -- Use working template
      v_payload,
      v_idempotency_key,
      auth.uid()
    )
