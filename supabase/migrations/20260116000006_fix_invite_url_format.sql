-- Migration: Fix invite_url format to avoid Resend flagging it as dangerous
-- Change from /invites?token= to /invites/ format
...
BEGIN
  ...
  -- Build URLs
  -- Changed: Use /invites/{id} instead of /invites?token={id}
  -- This format is less likely to be flagged as dangerous
  v_invite_url := v_base_url || '/invites/' || NEW.id::text;
  ...
  ELSE
    -- Officer invite - using cleaner URL format
    v_payload := jsonb_build_object(
      'club_id', NEW.club_id::text,
      'club_name', v_club_name,
      'club_slug', v_club_slug,
      'invite_id', NEW.id::text,
      'invite_url', v_invite_url,  -- Now: /invites/{id} instead of /invites?token={id}
