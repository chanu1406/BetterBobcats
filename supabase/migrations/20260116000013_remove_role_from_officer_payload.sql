-- Migration: Remove 'role' field from officer email payload
-- The role field might be causing Resend to reject the email
-- Use EXACT same payload structure as contact emails
...
BEGIN
  ...
  LOOP
    ...
    -- Use EXACT same payload as contact emails (no role field, no differences)
    ...
    BEGIN
      ...
      VALUES (
        ...
        v_payload,                -- Same payload structure (no role field)
        v_idempotency_key,
        auth.uid()
      )
