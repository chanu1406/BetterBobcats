...
import { updateClubRequest } from "../../actions";
...
interface EditClubRequestFormProps {
  requestId: string;
  initialData: {
    name: string;
    description: string;
...
/**
 * Client form component for editing a club request
 */
export default function EditClubRequestForm({
  requestId,
  initialData,
  majors,
}: EditClubRequestFormProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);

  // Form fields - initialized with existing data
  const [name, setName] = useState(initialData.name);
  const [description, setDescription] = useState(initialData.description);
  const [website, setWebsite] = useState(initialData.website);
  const [slugCandidate, setSlugCandidate] = useState(
    initialData.slug_candidate
  );
  const [contactEmail, setContactEmail] = useState(initialData.contact_email);
  const [officerEmails, setOfficerEmails] = useState(
    initialData.officer_emails.length > 0
      ? initialData.officer_emails
      : [""]
  );
  const [officerPhones, setOfficerPhones] = useState(
    initialData.officer_phones.length > 0
      ? initialData.officer_phones
      : [""]
  );
  ...
  // Majors
  const [isAllMajors, setIsAllMajors] = useState(
    initialData.is_all_majors || false
  );
  const [selectedMajorIds, setSelectedMajorIds] = useState<Set<string>>(
    new Set(initialData.major_ids)
  );
  const [majorNotes, setMajorNotes] = useState<Record<string, string>>(
    initialData.major_notes
  );
  ...
  const shouldAutoGenerateSlug = useRef(false);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        ...
        // Step 1: Update the club request
        const result = await updateClubRequest({
    ...
    <Card>
      <CardHeader>
        <div className="flex justify-between items-start">
          <div>
            <CardTitle>Edit Club Request</CardTitle>
            <CardDescription>
              Update your club request information
            </CardDescription>
          </div>
          <Link href="/clubs/my-requests">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <Input
                id="slug_candidate"
                value={slugCandidate}
                onChange={(e) => setSlugCandidate(e.target.value)}
                disabled={isPending}
